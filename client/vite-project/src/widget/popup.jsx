...
function Popup({ show, onClose, children, className = "", sabotageName }) {
  ...
  // Update the sabotage effect handler
  useEffect(() => {
    console.log("Sabotage name changed to:", sabotageName);
  ...
  return (
    <motion.div alt="popup" className={`${sabotage} fixed inset-0 bg-black/90 flex justify-center items-center z-50`}
      style={{
        x: driftX,
        y: driftY,
        filter: `${blurryActive ? 'blur(3px)' : ''} ${bugLampActive ? `brightness(${Math.max(1 - (bugsFinished * 0.05), 0.2)})` :
